...
type Patch struct {
	...
	// Absolute, if set, makes this patch override any existing value completely,
	// instead of performing a deep-merge for partial updates.
	Absolute bool
}

// ApplyEdits applies all patches in series to the given data.
func ApplyEdits(data string, edits ...Patch) (string, error) {
	for _, e := range edits {
		var err error
		if e.Value == nil ||
			// Check for typed nils
			(reflect.ValueOf(e.Value).Kind() == reflect.Ptr && reflect.ValueOf(e.Value).IsNil()) {
			data, err = Remove(data, e.Path)
			if err != nil {
				return "", errors.Wrap(err, e.Path)
			}
		} else {
			data, err = Edit(data, e.Value, e.Path)
			if err != nil {
				return "", errors.Wrap(err, e.Path)
			}
		}
